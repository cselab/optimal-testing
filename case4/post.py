...
####################################################################################################	
    ...
    ax.grid()
    ax.plot(dates ,np.sum(exact,axis=0),"--",label='reference')
    ...
    for result in results:
      ...
      #Find MAP
      logl       = result.logl
      parameters = samples[np.where( np.abs(logl-np.max(logl))<1e-18 )]
      parameters = np.reshape(parameters,(parameters.shape[1]))
      print (parameters)
      simulation = model(days,parameters,case)
      mmmm = np.zeros(days)
      for d in range ( days ):
          cases = simulation[d].Iu()
          mmmm[d] = np.sum(np.asarray(cases)) 
      #ax.plot(dates,mmmm,label=Name + ' map',color=color)
      ...
      ax.plot(dates,q50,'.',label=Name + ' median prediction' , color=color)
      #ax.plot(dates,np.mean(np.sum(prediction_matrix[jj],axis=2),axis=0),'--',label=Name + ' mean prediction',color=color)
      ax.fill_between(dates, qlo, qhi,label=Name + ' ' + str(100*p_conf)+"% credible interval",alpha=0.2,color=color)
      ...
      for i0 in range (6):
        for i1 in range (5):
          index = i0 * 5 + i1
          if index <= 25:
            axs[i0,i1].plot(dates,q50[:,index],label=Name + ' median prediction',color=color)
            axs[i0,i1].fill_between(dates, qlo[:,index], qhi[:,index],label=Name + ' ' + str(100*p_conf)+"% credible interval",alpha=0.2,color=color)
            ...
            if jj == 1:
              axs[i0,i1].plot(dates ,exact[index,:],"--",label='reference') 
            ...
            for k in range(points):
                  if canton_vector[k] == index:
                     axs[i0,i1].scatter(dates[int(day_vector[k])],case_vector[k],label='Artificial data ('+Name+")",color=color)
                     print("DATA",dates[int(day_vector[k])],case_vector[k],canton_vector[k])
    ...
    for result in results:
      ...
      for i0 in range (6):
        for i1 in range (5):
          ...
          if index <= 25:
            if jj == 1:
              ax3[i0,i1].plot(dates ,exact[index,:],"--",label='reference',color="green") 
            ...
            ax3[i0,i1].plot(dates,q50[:,index],label=Name + ' median prediction',color=color)
            ...
            for k in range(points):
                  if canton_vector[k] == index:
                     ax3[i0,i1].scatter(dates[int(day_vector[k])],case_vector[k],label='Artificial data ('+Name+")",color=color)
                     print("DATA",dates[int(day_vector[k])],case_vector[k],canton_vector[k])
            ...
            #ax3[i0,i1].plot(dates[0:days],q50_ ,"--",label=Name + ' median prediction',color=color)
